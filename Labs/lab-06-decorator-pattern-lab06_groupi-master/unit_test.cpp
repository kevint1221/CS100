...
TEST(DecoratorTestSet, ceilOnePointThree) {
    // Setup the elements under test
    Base* onePointThree = new Op(1.3);
    Ceil* test_ceil = new Ceil(onePointThree);
    EXPECT_EQ(test_ceil->evaluate(), 2);
}

TEST(DecoratorTestSet, floorOnePointThree) {
    // Setup the elements under test
    Base* onePointThree = new Op(1.3);
    Floor* test_floor = new Floor(onePointThree);
    EXPECT_EQ(test_floor->evaluate(), 1);
}

TEST(DecoratorTestSet, absFourPointThree) {
    // Setup the elements under test
    Base* fourPointThree = new Op(-4.3);
    Abs* test_abs = new Abs(fourPointThree);
    EXPECT_EQ(test_abs->evaluate(), 4.3);
}

TEST(DecoratorTestSet, Trunc_Test) {
    // Setup the elements under test
    ...
    add->operand1 = six;
    add->operand2 = seven;
    Trunc* test_trunc = new Trunc(add);
    EXPECT_EQ(14, test_trunc->evaluate());
}

TEST(DecoratorTestSet, Parenthesis_Test) {
    // Setup the elements under test
    SixMock* six = new SixMock();
    SevenMock* seven = new SevenMock();
    Add* add = new Add();
    add->operand1 = six;
    add->operand2 = seven;
    Paren* test_paren = new Paren(add);
    EXPECT_STREQ("(6.5+7.5)",ConvertStringToCharArray(test_paren->stringify()));
}
